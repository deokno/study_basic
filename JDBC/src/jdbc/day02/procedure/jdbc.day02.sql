...
begin
    ...
    from ( select stno, name, tel, addr
                ...
                , FK_CLASSNO
           from jdbc_tbl_student
           where addr like '%'|| p_addr_search ||'%') S 
...
/*
    ...
    월, 화, 수, 목, 금 만 가능하며, 또한 월, 화, 수, 목, 금이더라도
    시간이 오전 9시 부터 오후 1시 59분 59초 까지만 가능하도록 한다.
    insert 할 수 없는 요일명 이거나 시간대 이라면 "영업마감 이므로 insert를 할 수 없습니다." 이라는 오류메세지를 띄우도록 한다.
...
,p_fk_classno     IN  jdbc_tbl_student.fk_classno%type -- 학급번호
)
is
    v_day         varchar2(1);
    v_hour        varchar2(2);
...
begin
    -- 오늘의 요일명 알아오도록 한다.
    V_day := to_char(sysdate, 'd');  -- '1','2','3',....,'7'
                                     -- '일','월','화',...'토'         
    v_hour := to_char(sysdate, 'hh24');                                 
    if (v_day in('1','7') OR
        v_hour < '09' OR v_hour > '17' )then 
        raise error_dayTime;
    else 
    ...
    exception 
        when error_dayTime then
             raise_application_error(-20005, '>> 영업시간(월~금 09시~17시이전)이 아니므로 insert를 할 수 없습니다. <<');   
