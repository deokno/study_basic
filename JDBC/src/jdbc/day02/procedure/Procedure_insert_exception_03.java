	...
	begin
	    ...
	    exception 
	        when error_dayTime then
	             raise_application_error(-20005, '>> 영업시간(월~금 09시~14시이전)이 아니므로 insert를 할 수 없습니다. <<');   
	...
	public static void main(String[] args) {
         ...
         */
			...
			if (errorcode == 20005) {
				...
				System.out.println("학급번호 "+ fk_classno +"은 우리학교에 존재하지 않는 학급번호입니다. \n >>학급번호를 올바르게 입력하세요.<<");
			}
			else if(errorcode == 1) {
				System.out.println("입력하신 학번 "+stno+"은 사용중이므로 다른 학번을 입력하세요.");
			}
			else {
				e.printStackTrace();
			}
