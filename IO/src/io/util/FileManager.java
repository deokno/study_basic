...
public class FileManager {
		...
		try {
			...
			do {
				data = fr.read(); // fr 이라는 파일로 부터 글자(char)1개(2byte) 씩 읽어들임.
									// fr 이라는 파일로 부터 읽어들인 글자(char)가 없으면 -1을 리턴시켜줌.
				if (data != -1) { // 파일내용물에서 읽어온 것이 존재하는 경우
					sb.append((char) data);
				} else { // 파일내용물에서 더 이상 읽어올 것이 없는 경우
					break;
				}
			} while (true);
			// end of do~while
	...
	public static String reading_2(String fileName) throws FileNotFoundException, IOException {
		...
		do {
			...
			if (dataLength != -1) { // 파일내용물에서 읽어온 것이 존재하는 경우
