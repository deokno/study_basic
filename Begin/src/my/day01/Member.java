...
/*
	...
	"클래스"는 "필드(field)" == 속성(property) == 특성(attribute)" 와 
			...
			구성된다.
	>>> 추상화에 대해서 알아봅니다. <<<
	추상화란? 필요한 속성 및 기능만 뽑아내는 것을 맣한다. 
	
	※ 회원가입
	Member (병원) ==> 신장, 몸무게, 혈액형, 혈압, 등등
	Member (쇼핑몰) ==> 아이디, 비밀번호, 성명, 이메일, 전화번호, 주소, 마일리지 등등
...
*/
	...
	// id, pwd, name, email 은 다른 객체와 공유할 수 없는 변수이므로 instance(인스턴스)번수라고 부른다.
	
	// 2) static 변수 
	// 	  static 변수는 동일한 클래스로 생성되어진 서로 다른 instance(객체)등 끼리 공유하는 변수이다.
		static String address; // 주소
	
	// >> instance(인스턴스)번수 와 static 변수를 합쳐서 부를 때 member(구성원)변수라고 부른다.<<//
		
		
	//////////////////////////////////////////////////////////////
	
	//behavior == 행위 == operation == 기능 == 힘수(function) == 메소드(method)
	
	// ===회원의 정보를 화면에 출력해주는 기능을 만들어 봅니다. === //
	
	/*
	 	자바에서 소괄호()가 나오는 것을 오로지 2가지 밖에 없다. 
