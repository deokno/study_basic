...
public class GujikjaCtrl extends Controller implements InterGujikjaCtrl {
	...
	public void showAllGujikja_info(List<Member> mbrList) {
		...
		for(int i=0; i<mbrList.size(); i++) {
			...
			if(mbrList.get(i) instanceof Gujikja && !"admin".equals(mbrList.get(i).getId())) {
				count_gu ++;
			}
		...
		if(count_gu == 0 ) {
		   // mbrList 배열에 저장되어진 객체정보중 Gujikja 객체가 관리자(admin)만 저장된 경우
		   System.out.println(">> 등록되어진 구직자가 아무도 없습니다. << \n");	
		}
		
		else {
			// mbrList 배열에 저장되어진 객체정보가 관리자(admin) 이외에 일반 구직자도 저장된 경우
			System.out.println("-----------------------------------------------------------");
	...
	} // end of private void searchAgeLineGender()---------------
