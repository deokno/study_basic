...
public class LinkedList_main_3 {
	...
	public static void main(String[] args) {
		...
		/*
		 * [퀴즈 1] mbrList 에 저장되어진 Member 객체들 중에서 id 값이 "leess"인 회원만 그 회원의 정보를 출력하세요.
		 */
...
//   	String id = "superman";
		...
		System.out.println("\n~~~~~~~~~~~~~~~~ [퀴즈 2] ~~~~~~~~~~~~~~~~~~~~~~\n");
		/*
		 * [퀴즈 2] mbrList 에 저장되어진 Member 객체들 중에서 name이 "이"씨인 회원만 그 회원의 정보를 출력하세요.
		 * 
		 */
...
//	   	first_name = "서";
		...
		// index값이 없으면 mbrList의 맨 뒤에 추가된다. 
...
/*		for(int i=0; i<mbrList.size(); i++) { // mbrList.size() ==> 10
			...
			if(name != null  && name.startsWith(first_name)) {
									// 0 1 2 3(이순신) 4 5 7(이순신) 8(이민호) 9
				mbrList.remove(i);  // index 번호가 3을 삭제하면 
									// 0 1 2 4->3(김태희) 5->4(강호동) 7->6(이순신) 8->7(이민호) 9->8
									// index 번호가 6을 삭제하면 
									// 0 1 2 4->3(김태희) 5->4(강호동) 6(이민호) 7
									// 다음으로 for문의 증가식 i++로 인해 i는 6에서 7이 되는데  
									// i가 7을 포함해 그 다음으로는 "이"씨가 없으므로 remove(i); 하는 동작이 없게 된다.
									// 그래서 index 번호 6번으로 남은 "이민호"는 삭제되지 않고 남아있게 되는 것이다.
...
/*		
		for(Member mbr : mbrList) {
			...
			else mbrList.remove(first_name);	
		}
