...
public class Main01 {
	...
	void showInfo1() {
		...
		int hopemoney = 5000; 
		//지역변수는 반드시 초기화(어떤 변수에 처음부터 값을 부여해주는 것)를 해야한다.!!!
	...
	void showInfo2() {
		...
		// 지역변수는 반드시 초기화(어떤 변수에 처음부터 값을 부여해주는 것)를 해야한다.!!!
		// 지역변수는 { } 내에서만 사용되어지기 때문에 { }을 벗어나는 순간 지역변수는 자동적으로 메모리(RAM)에서 삭제가 되어진다.
		// 그러므로 showInfo1() 매소드 내에서 선언되어진 int hopemoney 지역변수와 showInfo2() 메소드내에서 선언되어진 int hopemoney 지역변수는 전혀 다른 것이다.
		System.out.println("===회원정보===\n"
				+ "1. 아이디 : " + id + "\n"
				+ "2. 함호 : " + pwd+ "\n"
				+ "3. 성명 : " + name + "\n"
	...
	/*
		//인스턴스에 할당되었던 메모리 공간을 바워야 한다. 
		ma1 = null;
		ma2 = null;
		
		System.gc(); // garbage collector (쓰레기 수집) 
	*/
	 // 위의 인스턴스에 할당되었던 메모리 공간을 비우는 것과 garbage collector (쓰레기 수집)를 하는 작업을 기술할 필요가 없다.
