...
public class Sungjuk {
	...
	/*
		...
		5. 예약어(예 package. import, class, String, float ...)는 변수명으로 사용불가하다.
		6. 필수사항은 아니지만 변수명명규칙의 관례인데 카멜표기법과 스네이크표기법을 따른다.
		
	 */
	
	String hakbun; // 학번 null   "12345"   "20191234"  (091234 int 사용 시 0을 인식을 못하기 때문에 사용하지 못함) 
					// 데이터 값이 숫자로만 이루어지더라도 맨 앞에 0 이 올 수 있는 경우라면 String타입으로 해야한다. 
	
	String name;  // 학생명 null 
	
	byte korPoint;  // 국어점수 0 카맬표기법 : 단어와 단어가 결합된 경우이라면 두번째 단어는 대문자로 시작하도록 하는 것.
	byte eng_point; // 영어점수 0 스네이크표기법 : 단어와 단어가 결합된 경우이라면 단어 사이에 _를 붙여주는 것.
	...
	// 성적 결과를 화면에 출력해주는 메소드(기능)를 만들고자 한다.
	void showSungjuk() {
		...
		// (short) (korPoint + eng_point + mathpoint)  ==> int 타입을 강제로 short 타입으로 형변환 시킨 것. 강제형변환(casting)이라고 부른다.
