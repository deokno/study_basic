...
public class CompanyCtrl {
	...
	private void showAllCompany_info(Member[] mbrArr) {
		...
		for(int i=0; i<Member.count; i++) {
			...
			if(mbrArr[i] instanceof Company && !"admin".equals(mbrArr[i].getId())) {
				count_com ++;
			}
		}
	...
	private void searchJobtype(Member[] mbrArr, Scanner sc) {
		...
		if(!jobtype.trim().isEmpty()) {
			...
			for(int i=0; i<Member.count; i++) {
				...
				if(mbrArr[i] instanceof Company && !"admin".equals(mbrArr[i].getId()))
				if(((Company) mbrArr[i]).getJobtype().toLowerCase().indexOf(jobtype.toLowerCase()) != -1) {
