...
public class OperatorMain {
	...
	public static void main(String[] args) {
		...
		System.out.println("n%3 =" + (n%3)); 			 // 10%3 = 1  10을 3으로 나누었을 때의 나머지 %는 정수일 때만 사용
		...
		/*
		  ...
		  int 는 4byte 이므로 32bit로 구성된다. 
		  10 << 3 은 정수 10을 이진수 00000000 00000000 00000000 00001010 으로 나타내고
		  00000 00000000 00000000 00001010___ 이 되는데
		  왼쪽 방향으로 3개를 밀어버린다.
		  그런 다음에 ___ 자리에 0으로 채운다. 
		  즉, 00000 00000000 00000000 00001010000이 된다.
		  이진수 00000000 00000000 00000000 01010000 이 되는데 이것을 십진수로 나타내면
		  1*(2의6승) + 1*(2의4승)
		    64 + 16 = 80 이 된다.
		  이것은 10*(2의3승) ==> 10(n)*(2의3승) 와 같은 것이 된다. 그러므로 80 이다.
		...
		/*
		  >>는 오른쪽 쉬프트 연산자
		   ...
		   int 는 4byte 이므로 32bit로 구성된다. 
		  10 >> 2 은 정수 10을 이진수 00000000 00000000 00000000 00001010 으로 나타내고
		  오른쪽 방향으로 2개를 밀어버린다.
		  __000000 00000000 00000000 000010 이 되는데
		  그런 다음에 __ 자리에 정수 10의 부호비트(가장 왼쪽 비트1개, 0이면 양수이고, 1이면 음수를 말하는 것이다.)와 같은 값으로 채운다. 
		  즉, 0000000000 00000000 00000000 000010 이 된다.
		  이진수 00000000 00000000 00000000 00000010 이 되는데 이것을 십진수로 나타내면
		  1*(2의1승) = 2 이 된다.
		  이것은 10*(2의2승) ==> 10(n)/*(2의2승) 와 같은 것이 된다. 그러므로 2 이다. 
		 */
		
		System.out.println("n>>>3 =" + (n>>>3)); 	// >>>는 오른쪽만 존재한다.		
	...
	/* 
		 ...
		 11111111 11111111 11111111 11110110 을 십진수로 나타내면 
	...
	/*
		  ...
		  int 는 4byte 이므로 32bit로 구성된다. 
		  -10 << 3 은 정수 -10을 이진수 11111111 11111111 11111111 11110110 으로 나타내고
		  왼쪽 방향으로 3개를 밀어버린다.
	...
	*/		
	...
	/*
		  ...
		  int 는 4byte 이므로 32bit로 구성된다. 
		  -10 >> 2 은 정수 -10을 이진수 11111111 11111111 11111111 11110110 으로 나타내고
		  오른쪽 방향으로 2개를 밀어버린다.
		  __11111111 11111111 11111111 111101 이 되는데
		  그런 다음에 __ 자리에 정수 -10의 부호비트와 같은 값으로 채운다.  
		  즉, 1로 채운다.
		  그러면 1111111111 11111111 11111111 111101이 된다.
		  이진수 11111111 11111111 11111111 11111101이 되는데 이것을 십진수로 나타내면
		  음수 이므로
	...
	*/
	...
	/*
		  ...
		  그런 다음에 __ 자리에 0 값으로 채운다.  
		  그러면 00011111111 11111111 11111111 11110이 된다.
		  이진수 00011111 11111111 11111111 11111110이 되는데 이것을 십진수로 나타내면
	    ...
	    */
			...
			/*
			  ...
			  11111111 11111111 11111111 11010101 <== ~42
				첫번째 비트는 부호비트로 사용되어지는데
				0은 +임(양수)을 의미하고,
				1은 -임(음수)을 의미한다.
	
				1010101
				
				첫번째 값만 -로 붙이고, 나머지 값은 +로 연산을 하면 된다.
